...
$$\begin{eqnarray}
\label{eqiv10}
\bm{y}&=&\bm{X}\bm{\beta}+\bm{u},\ \ \ E(\bm{u}\bm{u}^{T})=\sigma^{2}\bm{I}
\end{eqnarray}$$

where at least one of the explanatory variables in the $$n\times k$$ matrix $$\bm{X}$$ is assumed not to be predetermined with respect to the error terms.

We can form an $$n\times k$$ matrix $$\bm{W}$$ with typical row $$\bm{W}_{t}$$ such that all its elements belong to $$\Omega_{t}$$. The $$k$$ variables given by the $$k$$ columns of $$\bm{W}$$ are called \textbf{instrumental variables}, or simply \textbf{instruments}. The number of instruments may exceed the number of regressors.

Instrumental variables may be either exogenous or predetermined, and they should always include any columns of $$\bm{X}$$ that are exogenous or predetermined.
...
$$\begin{eqnarray}
\label{eqiv12}
\hat{\bm{\beta}}_{IV}&\equiv&(\bm{W}^{T}\bm{X})^{-1}\bm{W}^{T}\bm{y}
\end{eqnarray}$$

Whenever $$\bm{W}_{t}\in\bm{\Omega}_{t}$$
...
$$\hat{\bm{\beta}}_{IV}$$ is consistent and asymptotically normal under an identification condition. For asymptotical identification, this condition can be written as

$$\begin{eqnarray}
\label{eqiv14}
\bm{S}_{\bm{W}^{T}\bm{X}}&\equiv&\plim \frac{1}{n}\bm{W}^{T}\bm{X}
\end{eqnarray}$$
...
$$\begin{eqnarray}
\nonumber
\hat{\bm{\beta}}_{IV}&=&(\bm{W}^{T}\bm{X})^{-1}\bm{W}^{-1}\bm{y}\\
\nonumber
&=&(\bm{W}^{T}\bm{X})^{-1}\bm{W}^{-1}(\bm{X}\bm{\beta}_{0}+\bm{u})\\
\label{eqiv15}
&=&\bm{\beta}_{0}+(n^{-1}\bm{W}^{T}\bm{X})^{-1}n^{-1}\bm{W}^{T}\bm{u}
\end{eqnarray}$$


Given the assumption (\ref{eqiv14}) of asymptotic identification, $$\hat{\bm{\beta}}_{IV}$$ is consistent if and only if

$$\begin{eqnarray}
\label{eqiv15}
\plim \frac{1}{n}\bm{W}^{T}\bm{u}&=&\bm{0}
\end{eqnarray}$$
...
$$\begin{eqnarray}
\nonumber
Var\left(\plim_{n\to\infty}n^{1/2}(\hat{\be}_{IV}-\hat{\be})\right)&=&\sigma^{2}(\bm{S}_{\w^{T}\x})^{-1}(\bm{S}_{\w^{T}\w})(\bm{S}_{\w^{T}\x}^{T})^{-1}\\\nonumber
...
Let $$\bm{W}$$ denote an $$n\times l$$ matrix of instruments. 
\begin{enumerate}
\item $$l>k$$: \textbf{overidentified}
\item $$l=k$$: \textbf{just/exactly identified}
\item $$l<k$$: \textbf{underidentified}
\end{enumerate}
Method: seek an $$l\times k$$ matrix $$\bm{J}$$ such that the $$n\times k$$ matrix $$\bm{WJ}$$ is a valid instrument matrix, where $$\bm{J}$$ minimizes the asymptotic covariance matrix of the estimator in the class of $$IV$$ estimator obtained by instrument matrix $$\bm{W}\bm{J}^{\ast}$$ with arbitrary $$\bm{J}^{\ast}$$.

Three requirements for matrix $$\bm{J}$$
\begin{enumerate}
\item $$\bm{J}$$ has full column rank $$k$$
\item $$\bm{J}$$ is at least asymptotically deterministic
\item $$\bm{J}$$ is chosen to minimize the asymptotic covariance matrix of the resulting IV estimator
\end{enumerate}
...
\begin{enumerate}
\item [\textbf{Stage 1}] $$\x$$ is regressed on $$\w$$
\item [\textbf{Stage 2}]  The fitted values from the first-stage regressions to form the matrix $$\p_{\w}\x$$. Then the second-stage regression
...
$$\begin{eqnarray}
\label{eqiv19}
H_{0}&:&\y=\x\be+\bu, \ \bu\sim \mbox{IID}(\bm{0},\sigma^{2}\bm{I}),\ \ E(\x^{T}\bu)=\bm{0}\\
H_{0}&:&\y=\x\be+\bu, \ \bu\sim \mbox{IID}(\bm{0},\sigma^{2}\bm{I}),\ \ E(\w^{T}\bu)=\bm{0}
\end{eqnarray}$$
...
&=&(\x^{T}\p_{\w}\x)^{-1}\x^{T}\p_{\w}(\bm{I}-\x(\x^{T}\x)^{T}\x^{T})\y\\
...
where the $$k_{1}$$ columns of $$\bm{Z}$$ are included in the matrix of instruments $$\w$$, and $$k_{2}=k-k_{1}$$ columns of $$\bm{Y}$$ are treated as potentially endogeneous. OLS residuals are orthogonal to all the columns of $$\x$$, in particular to those of $$\bm{Z}$$

$$\begin{eqnarray*}
\bm{Z}^{T}\p_{\w}\m_{x}\y=(\p_{\w}\bm{Z})^{T}\m_{\x}\y=\bm{Z}^{T}\m_{\x}\y=\bm{0}
\end{eqnarray*}$$


We only need to test $$\bm{Y}^{T}\p_{\w}\m_{x}\y$$, which will not in general be identically zero under $$H_{1}$$, but should not differ from zero significantly under $$H_{1}$$.

The easiest way to test $$\bm{Y}^{T}\p_{\w}\m_{x}\y$$ is significantly different from zero is to use $$F$$ test for the $$k_{2}$$ restriction $$\bm{\delta}=\bm{0}$$ in the OLS regression

$$\begin{eqnarray}
\label{eqiv20}
\y&=&\x\be+\p_{\w}\bm{Y}\bm{\delta}+\bu
\end{eqnarray}$$


$$\begin{eqnarray*}
\hat{\bm{\delta}}&=&(\bm{Y}^{T}\p_{\w}\m_{\x}\p_{\w}\bm{Y})^{-1}\bm{Y}^{T}\p_{\w}\m_{x}\y
\end{eqnarray*}$$
