...
A typical row of this equation is $$\begin{eqnarray}
\label{eq4}
y_{t}&=&\bm{X}_{t}\beta+u_{t}=\sum_{i=1}^{k}\beta_{i}X_{ti}+u_{t}
\end{eqnarray}$$ where $$\bm{X}_{t}$$ denotes the $$t^{th}$$ row of $$\bm{X}$$
...
We partition $$\bm{X}$$ in terms of its columns as follows,
