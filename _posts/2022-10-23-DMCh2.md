...
where $$y$$ and $$\bm{u}$$
are $$n-$$vectors, $$\bm{X}$$ is an $$n\times k$$ matrix, and $$\beta$$ is
a $$k-$$vector.
...
$$\begin{eqnarray}
\label{eqp9}
\bm{x}^{T}_{i}(y-\bm{X\hat{\beta}})&=&\langle\bm{x}^{T}_{i},y-\bm{X\hat{\beta}}\rangle=0
\end{eqnarray}$$

([\[eqp9\]](#eqp9){reference-type="ref" reference="eqp9"}) means that
the vector $$y-\bm{X\hat{\beta}}$$ is orthogonal to all of the
...
$$\begin{eqnarray}
\label{eqp10}
\|y\|^{2}&=&\|\bm{X\hat{\beta}}\|^{2}+\|\bm{\hat{u}}\|^{2}
\end{eqnarray}$$
...
$$\begin{eqnarray}
\label{eqp11}
y^{T}y&=&\bm{\hat{\beta}}^{T}\bm{X}^{T}\bm{X}\bm{\hat{\beta}}+(y-\bm{X\hat{\beta}})^{T}(y-\bm{X\hat{\beta}})
...
$$y$$ into a vector of fitted $$\bm{X\hat{\beta}}$$ and a vector of
residuals $$\bm{\hat{u}}=y-\bm{X\hat{\beta}}$$. These mappings are
...
1.  $$\begin{eqnarray}
    \label{eqp13}
    \bm{X\hat{\beta}}&=&\bm{X}(\bm{X}^{T}\bm{X})^{-1}\bm{X}^{T}y=\bm{P_{X}y}\\\nonumber
    \hat{\bm{u}}&=&y-\bm{X}\hat{\beta}=(\bm{I-P_{X}})y=\bm{M}_{\bm{X}}y
    \end{eqnarray}$$ ([\[eqp13\]](#eqp13){reference-type="ref"
    reference="eqp13"}) says that projection matrix $$\bm{P_{X}}$$
    projects on to $$S(\bm{X})$$. For any $$n-$$vector $$y$$,
...
$$y=\bm{X\beta}+\bm{u}$$, the residuals and fitted values will not
...
$$\begin{eqnarray}
\label{eq20}
y&=&\bm{X}_{1}\beta_{1}+\bm{X}_{2}\beta_{2}+\bm{u}\\
\label{eq21}
\bm{M}_{1}y&=&\bm{M}_{1}\bm{X}_{2}\beta_{2}+\bm{M}_{1}\bm{u}
\end{eqnarray}$$
