...
Any $$n$$-vector $$\bm{X\beta}$$ belongs to $$S(x)$$, which is a
...
The estimator $$\bm{\hat{\beta}}$$ was obtained by solving the equation (\ref{eqp8}).

$$\begin{eqnarray}
\label{eqp8}
X^{T}(y-\bm{X\hat{\beta}})&=&\bm{0}
\end{eqnarray}$$
...
$$\begin{eqnarray}
\label{eqp9}
x^{T}_{i}(y-\bm{X\hat{\beta}})&=&\langlex^{T}_{i},y-\bm{X\hat{\beta}}\rangle=0
\end{eqnarray}$$
...
that $$\bm{\hat{u}}$$ is orthogonal to every vector in $$S(X)$$, the
span of the regressors.

The vector $$\bm{X\hat{\beta}}$$ is referred to as the vector of **fitted
values**, and it lies in $$S(X)$$. Consequently, $$\bm{X\hat{\beta}}$$
...
$$\begin{eqnarray}
\label{eqp11}
y^{T}y&=&\bm{\hat{\beta}}^{T}X^{T}X\bm{\hat{\beta}}+(y-\bm{X\hat{\beta}})^{T}(y-\bm{X\hat{\beta}})
...
$$\begin{eqnarray}
\label{eqp12}
\bm{P_{X}}&=&X\left(X^{T}X\right)^{-1}X^{T}\\
\nonumber
\bm{M_{X}}&=&\bm{I-P_{X}}=\bm{I}-X\left(X^{T}X\right)^{-1}X^{T}
\end{eqnarray}$$
...
1.  $$\begin{eqnarray}
    \label{eqp13}
    \bm{X\hat{\beta}}&=&X(X^{T}X)^{-1}X^{T}y=\bm{P_{X}y}\\\nonumber
    \hat{u}&=&y-X\hat{\beta}=(\bm{I-P_{X}})y=\bm{M}_{X}y
...
$$\begin{eqnarray}
    \bm{P}_{X}^{T}&=&\bm{P}_{X}\\
    \bm{P}_{X}\bm{P}_{X}&=&\bm{P}_{X}\\
    \bm{M}_{X}^{T}&=&\bm{M}_{X}\\
    \bm{M}_{X}\bm{M}_{X}&=&\bm{M}_{X}
...
4.  $$\bm{P_{X}}$$ and $$\bm{M_{X}}$$ annihilate each other
    ...
    points that lie in $$S(X)$$, and $$\bm{P_{X}}$$ likewise
...
5.  $$X=\left[\begin{array}{cc}
    ...
    $$\begin{eqnarray}
    \bm{P_{x_{1}}}&=&\bm{P_{X}}\\
    \bm{P_{x_{1}}}\bm{P_{X}}&=&\bm{P_{X}}\bm{P_{x_{1}}}=\bm{P_{x_{1}}}\\
    \bm{M_{x_{1}}}&=&\bm{M_{X}}\\
    \bm{M_{x_{1}}}\bm{M_{X}}&=&\bm{M_{X}}\bm{M_{x_{1}}}=\bm{M_{X}}
    \end{eqnarray}$$
...
\label{eq21}
\bm{M}_{1}y&=&\bm{M}_{1}X_{2}\beta_{2}+\bm{M}_{1}u
\end{eqnarray}$$
