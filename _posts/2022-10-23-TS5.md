...
For two members of $$C$$, $$x\in C$$ and $$y\in C$$, we need to say how close together they are. Technically, $$C$$ must be assigned a metric. For example, we can define **Euclidean metric** for any pair of real numbers $$x$$ and $$y$$ as $$d_{E}(x,y)=|x-y|$$. The pair $$(\mathbb{R},d_{E})$$ is known as the **Euclidean space**. 
