...
First differences $$\begin{aligned}
\Delta y_{t}&\equiv&y_{t}-y_{t-1}
\end{aligned}$$

Second differences $$\begin{aligned}
...
\end{aligned}$$ We know $$\begin{aligned}
...
The lag operator L is defined $$\begin{aligned}
L^{i}y_{t}\equiv y_{t-i}
\end{aligned}$$ Lag operator has following properties
...
6.  For $\left |a\right|<1$ $$\begin{aligned}
    \sum_{i=0}^{+\infty}(a^{i}L^{i})y_{t}&=&\frac{y_{t}}{1-aL}
    \end{aligned}$$

7.  For $\left |a\right|>1$ $$\begin{aligned}
    \sum_{i=0}^{+\infty}(a^{-i}L^{-i})y_{t}&=&\frac{-aLy_{t}}{1-aL}
    \end{aligned}$$

## First-Order Difference Equations

Consider the first-order difference equation below $$\begin{aligned}
y_{t}&=&\phi y_{t-1}+w_{t}
\end{aligned}$$ which can be written using the lag operator as
$$\begin{aligned}
y_{t}&=&\phi Ly_{t}+w_{t}\\
y_{t}-\phi Ly_{t}&=&w_{t}\\
(1-\phi L)y_{t}&=&w_{t}\\
(1+\phi L+\phi^{2}L^{2}+\cdots+\phi^{t}L^{t})(1-\phi L)y_{t}&=&(1+\phi L+\phi^{2}L^{2}+\cdots+\phi^{t}L^{t})w_{t}\\
\end{aligned}$$ The compound operator on the left-hand side is
$$\begin{aligned}
...
We have known that $$\begin{aligned}
(1+\phi L+\phi^{2}L^{2}+\cdots+\phi^{t}L^{t})(1-\phi L)y_{t}&=&y_{t}-\phi^{t+1}y_{-1}
\end{aligned}$$ if $|\phi|<1$ and $y_{-1}$ is finite, $\phi^{t+1}y_{-1}$
will become negligible as $t$ becomes large $$\begin{aligned}
(1+\phi L+\phi^{2}L^{2}+\cdots+\phi^{t}L^{t})(1-\phi L)y_{t}&\cong&y_{t}
\end{aligned}$$ A sequence $\{y_{t}\}_{t=-\infty}^{\infty}$ is bounded
if there exists a finite number $\bar{y}$ such that $|y_{t}|<\bar{y}$
for all $t$. When $|\phi|<1$ and when we are considering applying an
operator to a bounded sequence, we have $$\begin{aligned}
...
\end{aligned}$$ By this definition, we have $$\begin{aligned}
(1-\phi L)y_{t}&=&w_{t}\\
y_{t}&=&(1-\phi L)^{-1}w_{t}\\
...
Consider the second-order difference equation $$\begin{aligned}
y_{t}&=&\phi_{1}y_{t-1}+\phi_{2}y_{t-2}+w_{t}\\
(1-\phi_{1}L-\phi_{2}L^{2})y_{t}&=&w_{t}
\end{aligned}$$ The operator in the left-hand side contains a
...
\end{aligned}$$ Given values for $\phi_{1}$ and $\phi_{2}$, we seek
numbers $\lambda_{1}$ and $\lambda_{2}$ such that $$\begin{aligned}
\begin{cases}
\lambda_{1}+\lambda_{2}=\phi_{1}\\
\lambda_{1}\lambda_{2}=-\phi_{2}
\end{cases}
\end{aligned}$$ Two methods to consider this problem.

-   Method 1 $$\begin{aligned}
    (1-\phi_{1}z-\phi_{2}z^{2})&=&(1-\lambda_{1}z)(1-\lambda_{2}z)
    \end{aligned}$$ when $z=\lambda_{1}^{-1}$ or $z=\lambda_{2}^{-1}$,
    the right hand side is equal to 0. When $$\begin{aligned}
    \begin{cases}
    z_{1}=\frac{\phi_{1}-\sqrt{\phi_{1}^{2}+4\phi_{2}}}{-2\phi_{2}}\\
    \\
    z_{2}=\frac{\phi_{1}+\sqrt{\phi_{1}^{2}+4\phi_{2}}}{-2\phi_{2}}
    \end{cases}
    \end{aligned}$$ the left-hand side is equal to 0 as well. So if we
    let $$\begin{aligned}
    \begin{cases}
    ...
    \end{cases}
    \end{aligned}$$ both sides are equal to 0.
...
If $\lambda_{1}\neq \lambda_{2}$, we define $$\begin{aligned}
...
\end{aligned}$$ where $c_{1}=\lambda_{1}/(\lambda_{1}-\lambda_{2})$ and
