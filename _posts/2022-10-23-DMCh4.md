...
Consider Multivariable Normal Distribution $$\begin{aligned}
...
distribution, follows $$\begin{aligned}
...
1.  Let $\bm{z}=\bm{A}^{-1}\bm{x}$, where $\bm{A}\bm{A}^{T}=\Omega$
    ...
    &=&\bm{I}_{m}
    \end{aligned}$$ So $\bm{z}=\bm{A}^{-1}\bm{x}\sim N(\bm{0}, \bm{I})$
...
2.  Suppose $\bm{P}$ projects on to the span of the columns of an
    ...
    \end{aligned}$$ Let $\bm{x}=\bm{Z}^{T}\bm{z}$, and
    $\bm{x}\sim N(\bm{0},\bm{Z}^{T}\bm{Z})$. Therefore,
    $$\begin{aligned}
    \bm{z}^{T}\bm{P}\bm{z}&=&\bm{x}^{T}(\bm{Z}^{T}\bm{Z})^{-1}\bm{x}
    \end{aligned}$$ where $\bm{Z}^{T}\bm{Z}$ is the variance of
...
independent, then the random variable $$\begin{aligned}
\label{eqd4}
z&\equiv&\frac{z}{(y/m)^{1/2}}
\end{aligned}$$ is said to follow the **Student's t disribution** with
$m$ degrees of freedom. We write is as $$\begin{aligned}
t&\sim&t(m)
\end{aligned}$$
...
\end{aligned}$$ is said to follow the **F distribution** with $m_{1}$
and $m_{2}$ degrees of freedom. We write it as $$\begin{aligned}
F&\sim&F(m_{1}, m_{2})
\end{aligned}$$

# Tests of a Single Restriction

We want to test $\beta_{2}=0$ $$\begin{aligned}
\label{eq6}
...
and we obtain the test statistic $$\begin{aligned}
\nonumber
...
distribution with degree of freedom $(n-k)$. $$\begin{aligned}
\label{eq10}
...
\end{aligned}$$ where $\bm{X}_{1}$ is an $n\times k_{1}$ matrix,
...
\end{aligned}$$ Under the null hypothesis, this test statistic follow
the $F$ distribution with $r$ and $n-k$ degrees of freedom.

It is easy to find that $$\begin{aligned}
...
and the $USSR$ becomes $$\begin{aligned}
\nonumber
USSR&=&TSS-ESS\\\nonumber
...
reference="eq14"}) can be written as $$\begin{aligned}
\label{eq16}
...
\end{aligned}$$ where $\bm{\varepsilon}=\bm{u}/\sigma$
...
number of regressors. $$\begin{aligned}
...
\x_{2}
\end{array}\right]
\end{aligned}$$ Even we need different parameter vectors $\be_{1}$ and
$\be_{2}$ for two subsamples, we can nonetheless put the subsamples
together into a regression model $$\begin{aligned}
\label{eq25}
\left[\begin{array}{c}
\y_{1}\\
\y_{2}
...
\end{aligned}$$ where $\be_{1}+\bm{\gamma}=\be_{2}$.

We could rewrite ([\[eq25\]](#eq25){reference-type="ref"
reference="eq25"}) $$\begin{aligned}
\label{eq26}
\y=\x\be_{1}+\bm{Z}\bm{\gamma}+\bu&,&\bu\sim N(\bm{0},\sigma^{2}\bm{I})
\end{aligned}$$ The null hypothesis is $H_{0}:\bm{\gamma}=\bm{0}$ and it
...
\end{aligned}$$ where $x_{t}$ are **independent** random variables, each
...
\end{aligned}$$ is **asymptotically distributed** as $N(0,1)$. This
...
Suppose that the DGP is $$\begin{aligned}
\label{eq18}
\bm{y}&=&\bm{X\beta}_{0}+\bm{u}\\
\nonumber
\bm{u}&\sim&IID(\bm{0}, \sigma^{2}_{0}\bm{I})
\end{aligned}$$ We make another assumptions $$\begin{aligned}
\label{eq19}
E(u_{t}|\bm{X}_{t})&=&0\\
\nonumber
E(u_{t}^{2}|\bm{X}_{t})&=&\sigma^{2}_{0}
\end{aligned}$$ From the point of view of the error terms, it says that
...
explanatory variables is such that $$\begin{aligned}
\label{eq20}
\plim_{n\to\infty}\frac{1}{n}\bm{X}^{T}\bm{X}&=&\bm{S}_{\bm{X}^{T}\bm{X}}
\end{aligned}$$ where $\bm{S}_{\bm{X}^{T}\bm{X}}$ is a finite,
deterministic, positive definite matrix.

We rewrite $t_{\beta_{2}}$ as $$\begin{aligned}
\label{eq21}
...
asymptotically equivalent to $$\begin{aligned}
\label{eq22}
...
Under the null hypothesis, with exogenous regressors, $$\begin{aligned}
\label{eq23}
...
\end{aligned}$$ where $\bm{\varepsilon}=\bm{u}/\sigma_{0}$.
