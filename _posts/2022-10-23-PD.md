...
x_{it} & : & \text{a $$K\times1$$ vector of independent variables}\\
...
$$\begin{eqnarray*}
\mathbb{E}\left[y_{it}\midx_{it}\right] & = & \mathbb{E}\left[c_{i}\midx_{it}\right]+x_{it}^{\prime}\beta
\end{eqnarray*}$$


\begin{itemize}
- For the RE model we assume $$\mathbb{E}\left[c_{i}\midx_{it}\right]=\alpha$$,
so that 

$$\begin{eqnarray*}
\mathbb{E}\left[y_{it}\midx_{it}\right] & = & \alpha+x_{it}^{\prime}\beta
\end{eqnarray*}$$

and hence it is possible to identify $$\mathbb{E}\left[y_{it}\midx_{it}\right]$$.
- For FE model $$\mathbb{E}\left[c_{i}\midx_{it}\right]$$ varies
with $$x_{it}$$, and it is not known how it varies, so we cannot
identify $$\mathbb{E}\left[y_{it}\midx_{it}\right]$$. However,
we can estimate $$\beta$$ with short panels. Thus, it is possible
in the FE model to identify the marginal effect

$$\begin{eqnarray*}
\beta & = & \frac{\partial\mathbb{E}\left[y_{it}\mid c_{i},x_{it}\right]}{\partialx_{it}}
\end{eqnarray*}$$
