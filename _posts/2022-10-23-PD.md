...
If the model (\ref{eq:pd1}) is appropriate and $$Cov\left[u_{it},\bm{x}_{it}\right]=\bm{0}$$
then either $$N\to\infty$$ or $$T\to\infty$$ is sufficient for consistency. 

\subsubsection{Lest-squares Dummy Variable Estimator}
...
model (\ref{eq:pd2}) by OLS regression of $$y_{it}$$ on $$\bm{x}_{it}$$
and the $$N$$ individual dummy variables and yields the within estimator
for $$\bm{\beta}$$.

\subsubsection{Within Estimator}

Begin with the individual-specific effects model (\ref{eq:pd3}).
Taking the average over time yields $$\bar{y}_{i}=\alpha_{i}+\overline{\bm{x}}_{i}^{\prime}\bm{\beta}+\bar{\varepsilon}_{i}$$.
...
The \textbf{within estimator }is the OLS estimator in within model
(\ref{eq:pd6}) and it yields consistent estimates of $$\bm{\beta}$$
...
variation. Averageing over all years yields $$\bar{y}_{i}=\alpha_{i}+\overline{\bm{x}}_{it}^{\prime}\bm{\beta}+\bar{\varepsilon}_{i}$$,
which can be rewritten as the \textbf{between model}

$$\begin{eqnarray*}
\bar{y}_{i} & = & \alpha+\overline{\bm{x}}_{i}^{\prime}\bm{\beta}+\left(\alpha_{i}-\alpha+\bar{\varepsilon}_{i}\right)
\end{eqnarray*}$$

The \textbf{between estimator }is the OLS estimator from regression
of $$\bar{y}_{i}$$ on an intercept and $$\overline{\bm{x}}_{i}^{\prime}$$.
It uses variation between different individuals. 

The between estimator is consistent if $$\overline{\bm{x}}_{i}^{\prime}$$
...
is assumed to be correlated with $$\bm{x}_{it}$$.

\subsubsection{First-Differences Estimator}

Lagging one period of model (\ref{eq:pd3}) yields $$y_{it-1}=\alpha_{i}+\bm{x}_{it-1}^{\prime}\bm{\beta}+\varepsilon_{it-1}$$.
Subtracting this from $$y_{it}$$ yields the \textbf{first-differences
model
...
The \textbf{first-differences estimator }is the OLS estimator in (\ref{eq:pd7}).
...
\subsection{Panel Robust Statistical Inference}
...
\begin{enumerate}
\item \textbf{serially correlated}(correlated over $$t$$ for given $$i$$)
\item \textbf{heteroskedastic}
\end{enumerate}
we need to control these factors to get valid statistical inference.

\subsubsection{Panel Robust Sandwich Standard Errors}

The panel estimators which introduced above can be obtained by OLS
estimation of $$\bm{\theta}$$ in the pooled regression
...
$$\widetilde{y}_{it}$$, $$\widetilde{\bm{w}}_{it}$$, $$\widetilde{u}_{it}$$,
$$y_{it}$$, $$\bm{w}_{it}^{\prime}=\left[1\,\bm{x}_{it}^{\prime}\right]$$
...
The following matrix form equation illustrates the structure of $$\widetilde{\bm{y}}=\widetilde{\bm{w}}\bm{\theta}+\widetilde{\bm{u}}$$
