...
3.  Matrix form of a simple linear regression model $$\begin{aligned}
    \nonumber
    ...
    matrix, and $$\beta$$ a $$2$$-vector. $$\begin{aligned}
    ...
    \beta_{2}\\
    \end{array}\right]
    \end{aligned}$$ Equations ([\[eqm1\]](#eqm1){reference-type="ref"
    reference="eqm1"}) can be rewritten as $$\begin{aligned}
    \label{eqm2}
    y&=&X\beta+u
    \end{aligned}$$
...
4.  Multivariable linear regression model We have $k$ regressors and
    $\beta$$ now is $$k$$-vector $$\begin{aligned}
    y=\left[\begin{array}{c}
    y_{1}\\
    y_{2}\\
    \vdots\\
    ...
    A typical row of this equation is $$\begin{aligned}
    \label{eqm3}
    y_{t}&=&X_{t}\beta+u_{t}=\sum_{i=1}^{k}\beta_{i}X_{ti}+u_{t}
    \end{aligned}$$ where $X_{t}$ is the $t^{th}$ row of $X$.