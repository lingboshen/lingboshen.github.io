...
\begin{enumerate}
\item $$\Omega=\sigma^{2}I$$: (\ref{eq7.1}) is the classic linear regression model
\item $$\Omega$$ is diagonal with nonconstant diagonal elements: heteroskedastic
\item $$\Omega$$ is not diagonal
\end{enumerate}
...
This estimator is called the \textbf{generalized least squares}, or \textbf{GLS}, estimator of $$\beta$$
...
The generalized least squares estimator $$\hat{\beta}_{GLS}$$ can also be obtained by minimizing the \textbf{GLS criterion function}
...
$$\begin{eqnarray*}
\left((X^{T}\Omega^{-1}X)^{-1}\right)^{-1}&-&\left((W^{T}X)^{-1}W^{T}\OmegaW(X^{T}W)^{-1}\right)^{-1}\\
X^{T}\Omega^{-1}X&-&X^{T}W(W^{T}\OmegaW)^{-1}W^{T}X\\
\end{eqnarray*}$$
...
Steps to do FGLS
\begin{enumerate}
\item To obtain consistent estimates of $$\gamma$$, we must first obtain consistent estimates of the error terms in (\ref{eq7.14}).

\item Computing OLS estimates $$\hat{\beta}$$ allows us to calculate a vector of OLS residuals with typical element $$\hat{u}_{t}$$.
...
\item Feasible GLS of $$\beta$$ are obtained by using ordinary least squares to estimate regression (\ref{eq7.13}), with the estimates $$\hat{\omega}_{t}$$ replacing the unknown $$\omega_{t}$$. 
\end{enumerate}
...
\begin{enumerate}
\item Run OLS and compute OLS residuals $$\hat{u}=y-X
\hat{\beta}_{OLS}$$
...
If the $$u_{t}$$ follow a stationary AR(1) process, that is, if $$|\rho|<1$$ and $$Var(u_{t})=\sigma_{u}^{2}=\sigma_{\varepsilon}^{2}/(1-\rho^{2})$$, then the covariance matrix of the entire vector $$\hat{\bu}$$ is $$\Omega(\rho)$$ defined in (\ref{eq7.19}).
